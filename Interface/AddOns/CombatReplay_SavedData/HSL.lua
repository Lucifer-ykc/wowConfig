...
crHideFrames()
  ...
  crsaveddata[1].map={C_Map.GetBestMapForUnit("player"),C_Map.GetMapInfo(C_Map.GetBestMapForUnit("player")),mapwidth,mapheight}
  ...
  for i=1,GetNumGroupMembers() do
    local name,_,_,_,_,_,zone = GetRaidRosterInfo(i)
    if C_Map.GetMapInfo(C_Map.GetBestMapForUnit("player"))==zone or zone==myzone then
      --игрок в моей зоне
  ...
  crsaveddata[1].events[#crsaveddata[1].events].p={}
    ...
    for i=1,GetNumGroupMembers() do
      ...
      if playernr and playernr>0 then
        ...
        --координаты
        local x,y=C_Map.GetPlayerMapPosition(C_Map.GetBestMapForUnit("player"),"raid"..i)
      ...
      if UnitGUID(bossesid[bi]) then
        ...
        for n=1,#crsaveddata[1].boss do
          if UnitGUID(bossesid[bi])==crsaveddata[1].boss[n][1] then
            ...
            crsaveddata[1].events[#crsaveddata[1].events].b[n].po1=UnitPower(bossesid[bi], UnitPowerType(bossesid[bi]))
            crsaveddata[1].events[#crsaveddata[1].events].b[n].po2=UnitPowerMax(bossesid[bi], UnitPowerType(bossesid[bi]))
...
crClassColorID={"WARRIOR","DEATHKNIGHT","PALADIN","PRIEST","SHAMAN","DRUID","ROGUE","MAGE","WARLOCK","HUNTER","MONK","DEMONHUNTER"}
