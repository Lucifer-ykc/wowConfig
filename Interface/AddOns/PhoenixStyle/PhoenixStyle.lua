...
function pscheckzoneId(_zoneList, _zoneId)
...
function pscheckzoneId2(_zoneId,_zoneList)
...
pslocations={{282,{285,286},{294,295,296},{328,857},{367,368,369},{409,410,411,412,413,414,415}},{77777,{474,475},{471,472,473},{728,456},{508,509,510,511,512,513,514,515},{556,557,558,559,560,561,562,563,564,565,566,567,568,569,570}},{{610,611,612,613,614,615},{596,597,598,599,600},{661,662,663,664,665,666,667,668,669,670}},{{777,778,779,780,781,782,783,784,785,786,787,788,789},{764,765,766,767,768,769,770,771,772},{806,807,808},{850,851,852,853,854,855,856},99999999}}
...
psrecheckbossmin40=nil
	if UnitAffectingCombat("player") then
      ...
      if select(3,GetInstanceInfo())==17 or select(3,GetInstanceInfo())==18 then
        ...
        C_ChatInfo.SendAddonMessage("BigWigs", "VQ:0", "instance_chat")
        C_ChatInfo.SendAddonMessage("RW2", "^1^SReqVersionInfo^^", "instance_chat")
        --C_ChatInfo.SendAddonMessage("DXE", "^1^SRequestAddOnVersion^^", "instance_chat")
        C_ChatInfo.SendAddonMessage("DXE","DXEVersionRequest","instance_chat")
      else
        C_ChatInfo.SendAddonMessage("D4", "H\t", "RAID")
        C_ChatInfo.SendAddonMessage("BigWigs", "VQ:0", "RAID")
        C_ChatInfo.SendAddonMessage("RW2", "^1^SReqVersionInfo^^", "RAID")
        --C_ChatInfo.SendAddonMessage("DXE", "^1^SRequestAddOnVersion^^", "RAID")
        C_ChatInfo.SendAddonMessage("DXE","DXEVersionRequest","RAID")
      end
		end
	end
end
...
local inInstance, instanceType = IsInInstance()
if instanceType=="raid" and GetNumGroupMembers()>4 then
  local chatsen="RAID"
	...
	if IsAddOnLoaded("DBM-Core")==false then
		C_ChatInfo.SendAddonMessage("D4", "H\t", chatsen)
	end
	if IsAddOnLoaded("BigWigs")==false then
		C_ChatInfo.SendAddonMessage("BigWigs", "VQ:0", chatsen)
	end
	if IsAddOnLoaded("RaidWatch")==false then
		C_ChatInfo.SendAddonMessage("RW2", "^1^SReqVersionInfo^^", chatsen)
	end
		--у ДХе не проверяю установку ибо плохо работает пересылка версий и так
		--C_ChatInfo.SendAddonMessage("DXE", "^1^SRequestAddOnVersion^^", chatsen)
		C_ChatInfo.SendAddonMessage("DXE","DXEVersionRequest",chatsen)
...
if select(3,GetInstanceInfo())==17 or IsLFGModeActive(LE_LFG_CATEGORY_LFD) or IsLFGModeActive(LE_LFG_CATEGORY_SCENARIO) or select(3,GetInstanceInfo())==18 then
  C_ChatInfo.SendAddonMessage("PSaddon", "17"..psversion, "instance_chat")
else
  C_ChatInfo.SendAddonMessage("PSaddon", "17"..psversion, "raid")
end
end
end
end
...
local id=psGetUnitID(UnitGUID("boss1"))
    ...
    for e=1,#pslocations[d] do
      if pscheckzoneId(pslocations[d][e],C_Map.GetBestMapForUnit("player")) then
        bloc=1
      end
		end
	end
...
for i=1,#pslocations do
  for j=1,#pslocations[i] do
    if pscheckzoneId(pslocations[i][j],C_Map.GetBestMapForUnit("player")) then
      pslocationnames[i][j]=C_Map.GetMapInfo(C_Map.GetBestMapForUnit("player")).name
      pscurrentzoneid=j
      pscurrentzoneex=i
    end
	end
...
if psmsgmychat42=="instance_chat" then
  C_ChatInfo.SendAddonMessage("PhoenixStyle", "myname:"..psnamemsgsend.."++mychat:"..psmsgmychat42.."++", "instance_chat")
else
  if select(3,GetInstanceInfo())==17 or IsLFGModeActive(LE_LFG_CATEGORY_LFD) or select(3,GetInstanceInfo())==18 then
    C_ChatInfo.SendAddonMessage("PhoenixStyle", "myname:"..psnamemsgsend.."++mychat:"..psmsgmychat42.."++", "instance_chat")
  else
    C_ChatInfo.SendAddonMessage("PhoenixStyle", "myname:"..psnamemsgsend.."++mychat:"..psmsgmychat42.."++", "RAID")
  end
end
end
...
if psmsgmychat43=="instance_chat" then
  C_ChatInfo.SendAddonMessage("PhoenixStyle", "myname:"..psnamemsgsend.."++mychat:"..psmsgmychat43.."++", "instance_chat")
else
  if select(3,GetInstanceInfo())==17 or IsLFGModeActive(LE_LFG_CATEGORY_LFD) or select(3,GetInstanceInfo())==18 then
    C_ChatInfo.SendAddonMessage("PhoenixStyle", "myname:"..psnamemsgsend.."++mychat:"..psmsgmychat43.."++", "instance_chat")
  else
    C_ChatInfo.SendAddonMessage("PhoenixStyle", "myname:"..psnamemsgsend.."++mychat:"..psmsgmychat43.."++", "RAID")
  end
end
end
...
C_ChatInfo.SendAddonMessage("PhoenixStyle", "myname:"..psnamemsgsend.."++mychat:"..psmsgmychat.."++", chattt)
	else
C_ChatInfo.SendAddonMessage("PhoenixStyle", "myname:"..psnamemsgsend.."++mychat:"..psmsgmychat.."++fightend++", chattt)
	end
...
if ((arg2=="SPELL_CREATE" and (arg10==49844 or arg10==61031 or arg10==91756)) or (arg2=="SPELL_CAST_SUCCESS" and arg10==91756)) and pstrackbadsummons==1 then
...
C_ChatInfo.RegisterAddonMessagePrefix ("SAver")
C_ChatInfo.RegisterAddonMessagePrefix ("SAver2")
C_ChatInfo.RegisterAddonMessagePrefix ("PSaddmod")
...
C_ChatInfo.RegisterAddonMessagePrefix ("RW2")
C_ChatInfo.RegisterAddonMessagePrefix ("DXE")
...
local inInstance, instanceType = IsInInstance()
if instanceType~="pvp" then
C_ChatInfo.SendAddonMessage("PSaddon", "15PS "..UnitName("player").." v."..psversion.." "..psa1..psa2..psa3..pssaaddon_12[2].." rsc:"..psa5.." bsm:"..psa7..pcr.." chat:"..psa4..", "..psa6.." installed: "..psaddoninstalledsins, arg3)
end
end
...
local inInstance, instanceType = IsInInstance()
if instanceType~="pvp" then
C_ChatInfo.SendAddonMessage("PSaddon", "14PS "..UnitName("player").." v."..psversion.." "..psa1..psa2..psa3..pssaaddon_12[2].." rsc:"..psa5.." bsm:"..psa7..pcr.." chat:"..psa4..", "..psa6.." installed: "..psaddoninstalledsins, "WHISPER", arg4)
end
end
...
local _, _, day = tonumber(date("%H")),tonumber(date("%m")),tonumber(date("%d")),tonumber(date("%y"))
...
local inInstance, instanceType = IsInInstance()
if instanceType~="pvp" then
C_ChatInfo.SendAddonMessage("PSaddon", "17"..psversion, arg3)
...
if arg1=="SAver" then
	if pssaaddon_12[2]==1 and pssaaddon_12[3]==1 and IsAddOnLoaded("SayAnnouncer")==false then
		C_ChatInfo.SendAddonMessage("SAver2", "0PS", "WHISPER", arg4)
	end
end
...
if instanceType~="pvp" then
  if cchat==nil then
    if select(3,GetInstanceInfo())==17 or IsLFGModeActive(LE_LFG_CATEGORY_LFD) or IsLFGModeActive(LE_LFG_CATEGORY_SCENARIO) or select(3,GetInstanceInfo())==18 then
      C_ChatInfo.SendAddonMessage("PSaddon", "12info", "instance_chat")
    else
      C_ChatInfo.SendAddonMessage("PSaddon", "12info", "raid")
    end
  else
  C_ChatInfo.SendAddonMessage("PSaddon", "12info", cchat)
  end
end
end
end
...
if where==nil then
  if select(3,GetInstanceInfo())==17 or IsLFGModeActive(LE_LFG_CATEGORY_LFD) or select(3,GetInstanceInfo())==18 then
    C_ChatInfo.SendAddonMessage("PSaddon", "16off", "instance_chat")
  else
    C_ChatInfo.SendAddonMessage("PSaddon", "16off", "raid")
  end
else
C_ChatInfo.SendAddonMessage("PSaddon", "16off", where)
end
end
end
...
if IsInGuild() then
C_ChatInfo.SendAddonMessage("PSaddon", "17"..psversion, "guild")
end
	end
end
...
for t=1,#pslocations do
  for k=1,#pslocations[t] do
    if pscheckzoneId(pslocations[t][k],C_Map.GetBestMapForUnit("player")) then
...
for t=1,#pslocations do
  if #pslocations[t]>0 then
    for m=1,#pslocations[t] do
      if pscheckzoneId(pslocations[t][m],C_Map.GetBestMapForUnit("player")) then
        psmenuchoose1=m
