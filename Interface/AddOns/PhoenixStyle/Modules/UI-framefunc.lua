...
if UnitInRaid("player") and sa_delayforwaiting==nil then
  ...
  if select(3,GetInstanceInfo())==17 or IsLFGModeActive(LE_LFG_CATEGORY_LFD) or select(3,GetInstanceInfo())==18 then
    C_ChatInfo.SendAddonMessage("SAver", "1", "instance_chat")
	else
	  C_ChatInfo.SendAddonMessage("SAver", "1", "RAID")
	end
end
end
...
sayelldelay=0
	...
	if name==UnitName("player") then
		...
		C_ChatInfo.SendAddonMessage("SAYell", txt.."+0!"..chat.."+1!"..blocksec.."+2!"..important.."+3!", "WHISPER", name)
	end
end
